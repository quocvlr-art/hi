...
-- Auto change acc (logic don gian): acc cu chay -> du so mua box -> AutoBuyBox mua HET
-- so (240 so -> 2 box...). Khi mua het (so < gia box) va DA mo >=1 box -> call autoswap
-- 1 LAN roi doi acc:  ra Godly (Icecream) -> option havegodly; khong -> option no godly.
addTask("AutoChangeAcc", function()
	if not CFG.AutoChangeAcc or Runtime.SwapCalled then
		return
	end
	-- Da mo it nhat 1 box trong phien.
	if (Runtime.BoxesOpened or 0) < 1 then
		return
	end
	-- Da mua HET so (con < gia 1 box -> khong mua them duoc) moi doi.
	if getShells() >= (tonumber(CFG.BoxPrice) or math.huge) then
		return
	end
	...
	task.spawn(function()
		...
		pushLog("Mo box xong -> autoswap option "
...
local function createGui()
	...
	local perfLabel = makeRow(9, 22, "", Color3.fromRGB(140, 190, 170), 14, false)
	local statusLabel = makeRow(10, 44, "", Color3.fromRGB(255, 210, 105), 15, false)
	makeSeparator(11)
	local uptimeLabel = makeRow(12, 26, "", Color3.fromRGB(180, 190, 220), 18, true)
