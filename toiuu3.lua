...
local CFG = {
	...
	UseTween = true,
	TweenStudsPerSecond = 35, -- toc do tween (studs/giay); tang = nhanh hon
...
addTask("AutoChangeAcc", function()
	...
	pushLog("DU DIEU KIEN doi acc (xong daily + het so) -> cho 10s roi call autoswap")
	task.spawn(function()
		task.wait(10) -- dem 10s cho server luu/on dinh truoc khi doi acc
		if not ownsRuntime() then
			return
		end
		local ok = callAutoSwap(option)
		pushLog("Het 10s cho -> autoswap option "
