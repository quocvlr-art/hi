...
local CFG = {
	...
	-- Xong HET daily+weekly quest hom nay + DA mo box -> POST autoswap-complete de doi
	-- acc (tool move acc qua folder khac + tu keo acc moi vao). MAC DINH TAT (doi acc
	-- that, khong hoan tac). Bat = true khi da tao rule On-Demand tren web + dien ApiKey.
	AutoChangeAcc = false,
...
local Runtime = {
	...
	BoxesOpened = 0,
	GodlyReported = false,
	-- Auto change acc (autoswap): da call chua (1 lan/phien).
	SwapCalled = false,
	-- Server-hop check.
	HopCheckAt = nil,
	HopBaseline = 0,
...
local function onGodlyIcecream(itemId)
	...
	task.spawn(function()
		sendWebhook(("@everyone TRUNG GODLY **%s** !\nAcc: **%s** (@%s)\nBox: %s")
			:format(
				tostring(itemId),
...
-- Auto change acc + Print Daily Quest (chuẩn GUI): 
-- Check mốc DailyCoins (6 mốc). In tiến độ ra bảng log (chỉ in khi thay đổi để tránh spam).
-- Đổi acc nếu: hoàn thành xong TOÀN BỘ mốc (DailyCoins) VÀ không đủ tiền mua box.
local lastQuestProgress = -1
addTask("AutoChangeAcc", function()
	if Runtime.SwapCalled then
		return
	end

	-- Lấy thông tin Event + Profile
	local sharedEnv = ReplicatedStorage:FindFirstChild("SharedServices")
	if not sharedEnv then return end
	local EIS = require(sharedEnv:FindFirstChild("EventInfoService"))
	if not EIS or type(EIS.WaitForInitializedAsync) ~= "function" then return end
	
	local okEvent, mainEvent = pcall(function()
		return EIS:WaitForInitializedAsync():GetMainEvent()
	end)
	if not okEvent or not mainEvent or not ProfileData then
		return
	end

	local questConfigs = mainEvent.EventStartInfo.Quests
	local evntTitle = mainEvent.Title
	if not questConfigs or not ProfileData[evntTitle] then return end
	
	local playerQuests = ProfileData[evntTitle].Quests
	local questName = "DailyCoins"
	if not questConfigs[questName] then
		for k, v in pairs(questConfigs) do
			if tostring(k):lower():find("daily") or (v.Title and tostring(v.Title):lower():find("daily")) then
				questName = k
				break
			end
		end
	...
	local config = questConfigs[questName]
	local playerQuest = playerQuests and playerQuests[questName]
	...
	if config and playerQuest then
		...
		if progress ~= lastQuestProgress then
			...
			pushLog("Yêu cầu: " .. tostring(config.Description or "Collect coins"))
			pushLog("Tiến độ hiện tại: " .. progress)
			...
			pushLog(string.format("Số mốc hoàn thành: %d/%d", completedTiers, #config.Quests))
			
			if progress >= finalTarget then
				pushLog("Trạng thái: ĐÃ HOÀN THÀNH TOÀN BỘ")
			else
				pushLog("Trạng thái: VẪN CÒN LÀM ĐƯỢC")
				if nextTarget then
					pushLog("Mốc tiếp theo: " .. nextTarget)
					pushLog("Còn thiếu đến mốc tiếp theo: " .. (nextTarget - progress))
				end
				pushLog("Còn thiếu để hoàn thành toàn bộ: " .. (finalTarget - progress))
			end
		end
		...
		if progress >= finalTarget then
			if getShells() < (tonumber(CFG.BoxPrice) or math.huge) then
				if not CFG.AutoChangeAcc then return end
				...
				task.spawn(function()
					...
					pushLog("Xong Daily VÀ Sò không đủ mua box -> autoswap option "
	...
	return 15 -- Check thường xuyên hơn xíu để in progress kịp thời
end)
